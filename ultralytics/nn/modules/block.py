...
class FGlo_ECA(nn.Module):
    """
    FGlo-ECA: Efficient Channel Attention 作为 FGlo 替代（无 FC）
    仅建模通道依赖，不干扰空间结构，适合与 LADH/RGC 协同使用。
    """
    def __init__(self, channel, k_size=3):  # k_size 一般为 3 或 5
        super(FGlo_ECA, self).__init__()
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
    ...
    def forward(self, x):
        ...
        y = self.conv1d(y.squeeze(-1).transpose(-1, -2))  # [B, 1, C] -> [B, 1, C]
        y = self.sigmoid(y).transpose(-1, -2).unsqueeze(-1)  # [B, C, 1, 1]
        return x * y.expand_as(x)

class FGlo_GCT(nn.Module):
    """
    FGlo-GCT: Global Context Token 注意力机制
    参考 CVPR2020 GCT: https://arxiv.org/abs/2005.08757
    """
    def __init__(self, channel, epsilon=1e-5):
        super(FGlo_GCT, self).__init__()
        self.alpha = nn.Parameter(torch.ones(1, channel, 1, 1))  # 缩放
        self.gamma = nn.Parameter(torch.zeros(1, channel, 1, 1))  # 调节
        self.beta = nn.Parameter(torch.zeros(1, channel, 1, 1))   # 平移
        self.epsilon = epsilon

    def forward(self, x):
        # 计算 L2 范数（全局通道统计）
        norm = torch.sqrt(torch.sum(x ** 2, dim=(2, 3), keepdim=True) + self.epsilon)  # [B, C, 1, 1]
        context = self.alpha * norm  # 缩放 L2 norm
        gate = self.gamma * context + self.beta
        attention = torch.sigmoid(gate)
        return x * attention     
...
class SEAttention(nn.Module):
    def __init__(self, channel, reduction=16):
        super().__init__()
        self.avg_pool = nn.AdaptiveAvgPool2d(1)  # 压缩空间维度至1x1
        self.fc = nn.Sequential(
    ...
    def forward(self, x):
        ...
        y = self.avg_pool(x).view(b, c)
        # Excitation: 全连接层学习权重 [b,c] -> [b,c,1,1]
        y = self.fc(y).view(b, c, 1, 1)
        # Reweight: 通道加权
        return x * y.expand_as(x)
...
class ContextGuidedBlock_Down(nn.Module):
    ...
    def __init__(self, nIn, dilation_rate=2, reduction=16):
        ...
        self.F_glo = CoordAttention(nOut, reduction)
        #self.F_glo = FGlo_GCT(nOut)
